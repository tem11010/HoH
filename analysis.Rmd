...
A reviewer has asked about scale validity. 
...
1. Alpha, AVE, average inter-item correlations from CFA
2. Boxplot of Sum score QoL
3. T-test results for sum score
4. Wilcoxon results for individual scores
5. Anova table for sum score model with $R^2$ and f-value reported in text

## Supplement

1. Table of correlations of items with latent variable from cfa
2. Plot(s) of individual overall health responses -like the ones you have or like the one I include here
3. Tables of score change for individual items 
4. Anova table for individual items models 
...
bm0 <- ordbetareg(sum_post ~ sum_pre, data = na.omit(dat))
bm1 <- ordbetareg(sum_post ~ sum_pre + Era2 + Age +
                    Male.Female , 
                  data = na.omit(dat))
...
summary(bm1)
library(performance)
conditional_effects(bm1, "Era2")

compare_performance(bm0, bm1)
...
library(ordbetareg)
Overall.Health_post
bm0.o <- ordbetareg(Overall.Health_post ~ Overall.Health_pre, data = na.omit(dat))
bm1.o <- ordbetareg(Overall.Health_post ~ Overall.Health_pre + Era2 + Age + Male.Female , 
                  data = na.omit(dat))

summary(bm0.o)
summary(bm1.o)
library(performance)
conditional_effects(bm1.o, "Era2")

compare_performance(bm0.o, bm1.o)

hist(dat$sum_post)

bm0.qol <- ordbetareg(QoL_post ~ QoL_pre, data = na.omit(dat))
bm1.qol <- ordbetareg(QoL_post ~ QoL_pre + Era2 + Age + Male.Female , 
                  data = na.omit(dat))

summary(bm0.qol)
summary(bm1.qol)
library(performance)
conditional_effects(bm1.qol, "Era2")

compare_performance(bm0.qol, bm1.qol)
...
bm0.qosl <- ordbetareg(QoSL_post ~ QoSL_pre, data = na.omit(dat))
bm1.qosl <- ordbetareg(QoSL_post ~ QoSL_pre + Era2 + Age + Male.Female , 
                  data = na.omit(dat))

summary(bm0.qosl)
summary(bm1.qosl)
library(performance)
conditional_effects(bm1.qosl, "Era2")

compare_performance(bm0.qosl, bm1.qosl)
...
sum_dif <- dat$sum_post-dat$sum_pre


lm4 <-lm(sum_dif~Diagnosis2+Era2+Age+Male.Female, 
         data =dat)
summary(lm4)
car::Anova(lm4)
